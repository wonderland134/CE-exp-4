...
class calc_data():
	...
	def find_h(self):
		#del_h is positive value -> actual length is shorter than measured
		del_h_start = -3					#in cm
		del_h_end = 3
		del_h_del = 0.1
		...
		for i in range(len(del_h)):
			...
			if i>1:
				...
				if det<0:
					self.delta_h = np.round_(del_h[i],3)
...
if __name__ == '__main__':
	...
	print('')
	print('h deviation for each teams, delta_h is positive value -> actual length is shorter than measured')
	print('1st team : {}cm'.format(team_1st.delta_h))
	print('2nd team : {}cm'.format(team_2nd.delta_h))
	print('3rd team : {}cm'.format(team_3rd.delta_h))
	print('4th team : {}cm'.format(team_4th.delta_h))
	print('5th team : {}cm'.format(team_5th.delta_h))
